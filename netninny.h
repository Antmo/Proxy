...
class NinnyServer 
{
 ...
 private:
  ...
  int read_response(int);                 // Read HTTP response HEADER
  int fetch_response(int);                // Read whole response
...
/*
 * fetches the complete response for filtering purposes ^^
 */
int NinnyServer::fetch_response(int sockfd)
{
...
int NinnyServer::build_request()
{
  for ( auto str : BUFFER )
      ...
      while ( getline(ss,line) )
	{
	  ...
	  if (incase_find(line,"proxy-connection"))
	    continue;
